    ...
    $.fn.bootstrapDualListbox = function(options) {
        ...
        return this.each(function() {
            ...
            var settings = $.extend({
                ...
                //my code
                arrowbuttonstitles: ['Move all', 'Move selected', 'Remove selected', 'Remove all']   //string array
                        //my code
            }, options);
//            alert(settings.arrowbuttonstitles[0]);
            var container;

            if (settings.bootstrap2compatible) {
                container = $('<div class="row-fluid bootstrap-duallistbox-container bs2compatible"><div class="box1"><span class="info-container"><span class="info"></span><button type="button" class="btn btn-mini clear1 pull-right">' + settings.filtertextclear + '</button></span><input placeholder="' + settings.filterplaceholder + '" class="filter" type="text"><div class="btn-group buttons"><button type="button" class="btn moveall" title="' + settings.arrowbuttonstitles[0] + '"><i class="icon-arrow-right"></i><i class="icon-arrow-right"></i></button>' + (!settings.moveonselect ? '<button type="button" class="btn btn-default move" title="' + settings.arrowbuttonstitles[1] + '"><i class="icon-arrow-right"></i></button>' : '') + '</div><select multiple="multiple"></select></div><div class="box2"><span class="info-container"><span class="info"></span><button type="button" class="btn btn-mini clear2 pull-right">' + settings.filtertextclear + '</button></span><input placeholder="' + settings.filterplaceholder + '" class="filter" type="text"><div class="btn-group buttons">' + (!settings.moveonselect ? '<button type="button" class="btn btn-default remove" title="' + settings.arrowbuttonstitles[2] + '"><i class="icon-arrow-left"></i></button>' : '') + '<button type="button" class="btn removeall" title="' + settings.arrowbuttonstitles[3] + '"><i class="icon-arrow-left"></i><i class="icon-arrow-left"></i></button></div><select multiple="multiple"></select></div></div>').insertBefore($this);
            }
            else {
                container = $('<div class="row bootstrap-duallistbox-container"><div class="col-md-6 box1"><span class="info-container"><span class="info"></span><button type="button" class="btn btn-default btn-xs clear1 pull-right">' + settings.filtertextclear + '</button></span><input placeholder="' + settings.filterplaceholder + '" class="filter form-control" type="text"><div class="btn-group buttons"><button type="button" class="btn btn-default moveall" title="' + settings.arrowbuttonstitles[0] + '"><i class="icon-arrow-right"></i><i class="icon-arrow-right"></i></button>' + (!settings.moveonselect ? '<button type="button" class="btn btn-default move" title="' + settings.arrowbuttonstitles[1] + '"><i class="icon-arrow-right"></i></button>' : '') + '</div><select multiple="multiple" class="form-control"></select></div><div class="col-md-6 box2"><span class="info-container"><span class="info"></span><button type="button" class="btn btn-default btn-xs clear2 pull-right">' + settings.filtertextclear + '</button></span><input placeholder="' + settings.filterplaceholder + '" class="filter form-control" type="text"><div class="btn-group buttons">' + (!settings.moveonselect ? '<button type="button" class="btn btn-default remove" title="' + settings.arrowbuttonstitles[2] + '"><i class="icon-arrow-left"></i></button>' : '') + '<button type="button" class="btn btn-default removeall" title="' + settings.arrowbuttonstitles[3] + '"><i class="icon-arrow-left"></i><i class="icon-arrow-left"></i></button></div><select multiple="multiple" class="form-control"></select></div></div>').insertBefore($this);
            }
...
//                removebutton: $('.box2 .remove', container),
            ...
            {
                elements['movebutton'] = $('.box1 .move', container);
                elements['removebutton'] = $('.box2 .remove', container);
            }
            init();
            ...
            function init()
            {
                ...
                //my code
                var originalselectid = elements.originalselect.attr('id') || '';
                //my code
                ...
                if (!!settings.helperselectnamepostfix)
                {
                    ...
                    //my code
                    elements.select1.attr('id', originalselectid + settings.helperselectnamepostfix + '1');
                    elements.select2.attr('id', originalselectid + settings.helperselectnamepostfix + '2');
                }
            ...
            function refreshselects()
            {
                ...
                elements.originalselect.find('option').each(function(index, item) {
                    ...
                    if ($item.prop('selected')) {
                        selectedelements++;
                        elements.select2.append($item.clone(true).prop('selected', settings.preserveselectiononmove ? 'selected' : false/*$item.data('_selected')*/));
                    }
                    else {
                        elements.select1.append($item.clone(true).prop('selected', settings.preserveselectiononmove ? 'selected' : false/*$item.data('_selected')*/));
                    }
                });
