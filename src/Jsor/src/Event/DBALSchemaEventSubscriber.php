...
class DBALSchemaEventSubscriber implements EventSubscriber {
    ...
    public function postConnect(ConnectionEventArgs $args) {
        if ($this->postConnectCalled) {
            ...
            sprintf(
                    'It looks like you have registered the %s to more than one connection. Please register one instance per connection.', get_class($this)
            )
            );
        }
    ...
    public function onSchemaCreateTable(SchemaCreateTableEventArgs $args) {
        $generator = new CreateTableSqlGenerator(
                $args->getPlatform(), $this->schemaManager->isPostGis2()
        );

        $args
                ->addSql(
                        $generator->getSql(
                                $args->getTable(), $args->getColumns(), $args->getOptions()
                        )
                )
                ->preventDefault()
        ;
    ...
    public function onSchemaAlterTableRemoveColumn(SchemaAlterTableRemoveColumnEventArgs $args) {
        ...
        if ($column->getNotnull()) {
            ...
            $args->addSql(sprintf(
                            'ALTER TABLE %s ALTER %s SET DEFAULT NULL', $table->getQuotedName($platform), $column->getQuotedName($platform)
            ));
        }

        // We use DropGeometryColumn() to also drop entries from the geometry_columns table
        $args->addSql(sprintf(
                        "SELECT DropGeometryColumn('%s', '%s')", $table->getName(), $column->getName()
        ));
...
//        die();
        ...
        if ($columnDiff->hasChanged('srid')) {
            $args->addSql(sprintf(
                            "SELECT UpdateGeometrySRID('%s', '%s', %d)", $table->getName(), $column->getName(), $column->getCustomSchemaOption('srid')
            ));
        }
    }
    ...
    public function onSchemaIndexDefinition(SchemaIndexDefinitionEventArgs $args) {
        ...
        $spatialIndex = new Index(
                $index['name'], $index['columns'], $index['unique'], $index['primary'], array_merge($index['flags'], array('spatial'))
        );
